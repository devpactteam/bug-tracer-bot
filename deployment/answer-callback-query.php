...
if ($token === '' || ! is_string($callbackQueryId) || $callbackQueryId === '') {
    gatewayLog('request.validation_failed', [
        'has_token' => $token !== '',
        'has_callback_query_id' => is_string($callbackQueryId) && $callbackQueryId !== '',
    ]);
...
gatewayLog('telegram.parameters.prepared', [
    'has_callback_text' => isset($parameters['text']),
    'callback_text_length' => isset($parameters['text']) ? strlen($parameters['text']) : 0,
]);
